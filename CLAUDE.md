# CLAUDE.md — pyf 的 AI 组织控制面（军团运行时之家）
...
## 你服务于谁
pyf。契约：**默认放行、异常才找、RED 才打断**。pyf 只做四件事——写文档 / 拍任务 / IM 审批 / 和 PM 聊。

## 事实源（记忆在文件里，不在聊天历史里）
- **任务/状态**：GitHub Issues + Projects v2（label 路由 `auto-eligible` / `red-line` / `project:*`）
- **知识库**：`~/army/pyf-docs/`（PRD/决策/runbook；PM 读 PRD、写 briefing/reports）
...
- 出口探活：`~/army/bin/army-watchdog`（DOWN = Windows 侧代理挂了，全军断 API）

## 红线（硬约束，详见 `docs/red-lines-spec.md`）
- **RED（永远人 ack）**：push main/master、改 .env/secret/删 prod、≥10MB 进 marginalia、bili 601 重试、tv 跳 AIGC 备案
- **预算闸**：video-dreamina 失败不自动抽卡
- 第一周 hook 只 log 不拦

## 工作路径铁律
- `~/claw` 根【永不】作工作路径（不在 claw 根跑 git）；只在【具体项目路径】里干活
- worker 永远 `--cwd <项目路径>`（自动加载该项目自己的 CLAUDE.md）

## 上下文经济
事件式、状态外置、Worker 默认串行吃 cache（详见设计文档 §5）。干完即 PERSIST 写回事实源，session 可弃。
